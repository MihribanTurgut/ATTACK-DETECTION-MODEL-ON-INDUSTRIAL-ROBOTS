...
X_train_reshaped = X_train_scaled_filtered.reshape(X_train_scaled_filtered.shape[0], X_train_scaled_filtered.shape[1], 1)
X_test_reshaped = X_test_scaled_filtered.reshape(X_test_scaled_filtered.shape[0], X_test_scaled_filtered.shape[1], 1)

#rnn model
model = Sequential()
model.add(SimpleRNN(units=128, return_sequences=True, input_shape=(X_train_scaled_filtered.shape[1], 1)))
...
model.summary()
model.save('RNN.h5')
history = model.fit(X_train_scaled_filtered, y_train, epochs=150, batch_size=64, validation_split=0.2)
